...
using SpecialFunctions        # besselj, besselj_zero
...
Uses the identity: zeros of j_l = zeros of J_{l+1/2}
(both are positive real, j_l(x) = √(π/2x) J_{l+1/2}(x) with the same zeros).
"""
jl_zero(l::Int, n::Int) = besselj_zero(l + 0.5, n)
...
"""
function check_physical_roundtrip(b::BesselBasis; f=r -> exp(-r) * sin(π * r / b.R))
    u_true = f.(b.r)
...
"""
    check_derivative(b; f, df) → err

Verify spectral differentiation against a known derivative.
"""
function check_derivative(b::BesselBasis; f=r -> sin(π * r / b.R), df=r -> (π / b.R) * cos(π * r / b.R))
    û = to_spectral(b, f.(b.r))
    du_spec = deriv_collocation(b, û)
    du_true = df.(b.r)
    return norm(du_spec - du_true) / norm(du_true)
end
...
function run_tests()
    ...
    println("Orthogonality error  ‖G − I‖∞  (N=20, R=1, N_quad=2N+10)")
    println("-" ^ 50)
