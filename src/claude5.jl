...
"""
function check_physical_roundtrip(b::BesselBasis; f=r -> (r / b.R)^b.l * cos(π * r / (2b.R)))
    u_true = f.(b.r)
...
function run_tests()
    ...
    println()
    println("Physical roundtrip: algebraic rate verification (l=2, R=1)")
    println("f = sin(πr/R)^{l+p} * exp(-r/R),  theory: error ~ O(1/N^{2p+1})")
    println("-" ^ 64)
    Ns = [10, 20, 40, 80]
    @printf("  %4s", "p")
    for N in Ns
    ...
    @printf("  %6s  %8s\n", "ratio", "theory")
    for p in 1:3
        errs = [check_physical_roundtrip(BesselBasis(2, N, 1.0); f=r -> sin(π*r/1.0)^(2+p) * exp(-r/1.0)) for N in Ns]
        @printf("  %4d", p)
        for e in errs
            ;
            @printf("  %10.2e", e);
        end
        @printf("  %6.1f  2^%d=%d\n", errs[end - 1]/errs[end], 2p+1, 2^(2p+1))
    end
